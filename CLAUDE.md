...
| **0 — OWNER RULING** | ISC-1 Inventory sheet cleanup — ANALYSIS + PLAN only (08-Sep-2026) from the 04-Sep PDF: 105 sold rows carry a text SoldDate (`cashmere …`) the bot cannot date; 2,853 legacy rows have no `bale_uid` so **sorting the sheet re-keys them — do not sort until the backfill runs**; 3 customer spelling clusters (Awunawu ×3, Oshodi ×4, Ketu ×2); 374 SOLD rows at ₦0; 46 oversize thans for EDB-1; one duplicate uid on 77014/864. Owner: run `scripts/format-date-columns.js --commit` (still pending since 14-Aug), answer R1–R11 in the spec, then the guarded one-offs ship in order. | **Owner** | `telegram-ops-bot/specs/ISC-1_INVENTORY_SHEET_CLEANUP.md` §4–§5 |
