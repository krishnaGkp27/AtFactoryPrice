...
| **0 — OWNER RULING** | PAY-3 the finance phone — PROPOSAL only (14-Sep-2026, owner: "it doesn't show me the date when it was requested… this is also the same phone where the employer is getting a request for making payment"). Four faults on the payment approval card as seen on the office phone: **F1** no request DATE (only `1d ago`) although the finance card that follows it prints `13-Sep-2026, 11:39`; **F2** the 1-of-2 note names no signer, so on a phone that is also an admin you cannot tell whether the existing signature is your OWN until the tap is refused; **F3** the "needs a SECOND" alert prints the raw UUID (`4f72be16-…`) while the card below calls it `R-4F72`, and names no signer, amount or payee; **F4** the chips say `13 Sept · request payment · Abdul` — no amount, the one fact an approver scans for. Proposed output written verbatim in the spec §2. **Menu answer: approve in 🛂 Approvals → 💳 Payments, pay in 💰 Finance → 💳 Payments → 💳 Waiting for me to pay — a request is never in both at once** (it leaves the approval queue on the second signature). ~27 lines; three edits are inbox-only, the alert touches `approvalEvents` (ask-first). **R1 owed: may the admin who gives the second signature also be the one who pays?** Today yes — `canExecute` checks only finance-seat membership. (a) leave it *(recommended at four people)*, (b) the finance seat may never sign, (c) it may give the first signature but never the second. | **Owner** | `telegram-ops-bot/specs/PAY-3_FINANCE_PHONE.md` §2, §4 |
