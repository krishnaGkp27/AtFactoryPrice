...
| **0 — OWNER LAYOUT GO** | CUR-2 invoice rate multiplier — LAYOUTS drawn for the owner (09-Sep-2026): the sale wizard's optional Step 5 (No multiplier · Settings value · type), the invoice PDF in both states on one worked example (entered 3.20/yd unconverted vs × 1,250 = 4,000.00/yd, every figure incl. the red payment row and DEBIT BALANCE), staff cards stay in variable 1 with one "Customer copy × 1,250" line, new trailing `Invoices.rate_multiplier` column (freeze at issue), Settings `INVOICE_RATE_MULTIPLIER` + `INVOICE_MULTIPLIER_ASK`. Owner: approve the drawings and answer Q1–Q6 + D7 (each recommended). Build waits for that go; steps 4–5 touch approvalEvents (ask-first). | **Owner** | `telegram-ops-bot/specs/CUR-2_INVOICE_MULTIPLIER.md` §3b, §4, §10 |
| **0 — OWNER RULING** | CUR-1 currency display — PROPOSAL only (08-Sep-2026): rule §17 mapped onto every money site (≈95 expense sites keep ₦, ≈210 sale/inventory sites go bare, ≈75 undecided with a recommended side). One money module + a smoke lint + a mechanical sweep; conflicts flagged with INV-1 decision 9 and the in-flight INV-2 invoice brief ("₦ per yard") — **amend the brief before the design is finalised (R1)**. Owner: answer rulings R1–R18 in the spec (each has a recommended answer; "as recommended" closes them all). | **Owner** | `telegram-ops-bot/specs/CUR-1_CURRENCY_DISPLAY.md` §10 |
