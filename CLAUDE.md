...
| **0 — OWNER STEP** | RET-3 SHIPPED 02-Sep-2026: an approved return now credits the buyer at the booked rate (was ₦0 on every return since returns went behind approval). Owner: run `node scripts/list-uncredited-returns.js` (read-only) and rule on the backfill of past returns; approve one small return live and check the statement moves. Then answer the 5 RET-4 queries (return card: customer → bale → thans → date → condition → photo → confirm; dual-admin kept) so the card can be built. | **Owner** | `telegram-ops-bot/specs/RET-3_RETURN_CREDIT.md` |
