...
| **0 — OWNER TEST** | EDB-1 ✏️ Edit Bale SHIPPED 02-Sep-2026: the bale card edited in place (design · shade · indent · yards per than · add a than), label photo required, one dual-admin `edit_bale` approval, executor re-checks the live rows then updates cells / appends thans on Inventory. Owner: run the 6061 correction through it (60 → 30, add a 30) — 3-minute steps in the spec. Deferred by ruling: removing a than (shape undecided), the ₦105,000 Qaribullah credit (financial reconciliation later). | **Owner** | `telegram-ops-bot/specs/EDB-1_EDIT_BALE.md` |
