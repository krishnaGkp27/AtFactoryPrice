...
| **0 — OWNER RULING** | STS-1 "where is it pending, and with whom?" — AUDIT + PROPOSAL (14-Sep-2026), written from the same question that produced TRF-19/DEC-1. Scores every multi-stage workflow on whether ANY screen says the stage and the actor after the push message scrolls away. **Tasks 3/3 (the shape to copy); returns, goods receipts and Edit Bale score 0/3** — dual-admin, ledger- or Inventory-changing, and invisible once the submit message scrolls away; supply requests are 3/3 on their own tile but 1/3 on the card that carries the buttons. Five cheap fixes (dual note names the first signer, `1/2` on dual chips, stage+holder on the supply card, order holder phrased as the salesperson, payment status line on pending rows) need no ruling. Three do: **R1** one shared `📋 My requests` over ApprovalQueue by `user` (closes returns + GRN + Edit Bale at once — recommended yes; owner decides whether an admin's copy lists everything or only their own), **R2** a stage guard on the supply approve path (`approvalEvents`, ask-first file — recommended yes), **R3** stage + signers on the /ops web API (recommended later). No new sheet, column or table; no approval-semantics change. | **Owner** | `telegram-ops-bot/specs/STS-1_WHO_HOLDS_IT.md` §3 |
