...
| **0 — OWNER GO** | PAY-2 payment reason — PROPOSAL (09-Sep-2026): every payment request carries a typed reason (3–120 chars, after the amount) shown on the confirm, approval and finance cards; stored in a new Postgres table `payment_reasons` (migration `002`) with the payload copy on ApprovalQueue as fallback and an optional mirror column S on `PaymentRequests`; phase 2 = the requester's repeating reasons become chips; phase 3 = a reason-code index + analysis + export. Owner: answer Q1–Q7 (each recommended) and say go. | **Owner** | `telegram-ops-bot/specs/PAY-2_PAYMENT_REASON.md` |
