...
|---|---|---|---|
| **0 — NEXT BUILD** | VRF-3: bill check keys on the GOODS — thans-only sales skip the OCR entirely (credit saved, no 🔬 noise); any complete bale still verifies; mixed sales verify bales only. Spec FULLY LOCKED — build without re-asking. | **Agent** | `telegram-ops-bot/specs/VRF-3_THAN_SALE_BILL_SKIP.md` |
