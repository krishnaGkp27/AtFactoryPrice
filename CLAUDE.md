...
| **0 — OWNER LIVE CHECK** | CART-PEEK SHIPPED 16-Sep-2026 (owner: "when I am selecting the design → shade → quantity from the supply request, I am not able to see what I have selected in the cart already"). Every supply-request PICKER now carries the basket: the design list (replacing the bare `🛒 N in cart`), the shade picker in all four of its renders (album picker, morph-back caption, photo-combo caption, text fallback), the quantity card (block sits ABOVE "How many bales?" so the question stays nearest the chips), and the typed Custom Quantity prompt. The block is the cart card's own lines under a tally header — `🛒 In cart · Σ 3B` / `202/201` / `  • 3 - Navy Blue · 2B` — drawn by the same `cartFormat` the six UX-2 cards use (`formatCartPeek`), so the peek and the cart can never disagree. Photo captions are capped at 1,024 chars, so the block caps at 8 lines with a `…more in 🛒 Back to cart` pointer and, past 420 chars, collapses to the tally line alone — never cut silently. Empty cart = byte-identical screens to before. Hardened the same day through TWO adversarial review rounds (names Markdown-escaped — and, round 2, the cart card / confirmation / receipt now escape the SAME rows via `supplyCartRowsMd`, closing a pre-existing gap the pointer led into; the two photo captions pass the ROOM LEFT under 1,024 as the block's budget — measured: 16 overflowing shades = full block at 1,021, 18 = tally line, 20 = nothing; the sold-out shade note and the typed re-prompts keep the basket; pointer reads `…and more in the cart`; the cut never strands a design header; `]` deliberately not escaped — legacy Markdown renders that backslash). Controller: surgical insertions only, no flow logic touched. **Owner: from the supply request add one shade of 202/201, tap ➕ Add More, and confirm the shade picker's photo caption and the next quantity card both list the line you already have.** | **Owner** | `telegram-ops-bot/specs/CART-PEEK.md`; `src/utils/cartFormat.js` (`formatCartPeek`), `telegramController` (`cartPeek`) |
| **0 — OWNER LIVE CHECK** | RATE-1 SHIPPED 21-Sep-2026 (owner 17-Sep: "there is a suggestion before this chip for the last sold rate but still I am getting a request to type in the rate again"). The Step 2 `last paid by <customer>` chip read Transactions and matched on the design column — but a BUNDLE sale (the main door) writes that row with an EMPTY design and only the first design's rate, so the chip never appeared for bundle buyers (and the 400-row read cap hid older sales). Now the lookup reads the **approved request's own row first** — the executor's `enrichment.ratePerUnitByDesign` (every sale since INV-1a, 14-Jul) before the wizard's `enrichDraft` (APC-1, 08-Aug); exact per design (an un-keyed design mirrors the executor's first-rate fallback only when `yardsByDesign` says it was sold); newest approval by resolved time; every CUS-2 spelling; sales undone by an approved `revert_sale_bundle` excluded; no row cap; resolved rows memoised 30 s — Transactions kept as the legacy fallback; a failing queue read falls through, never throws. With one design and no hit the card now says `No earlier sale of 77019 to ABBA found.` (escaped — a `_` in a name would otherwise pair with the italic footer and blank the card) above the typed-reply footer instead of silently omitting the chip; a chipless re-render clears `state.lastPaidRate` so an old card's chip cannot book the previous buyer's rate. **Hardened the same day after an adversarial review (nine findings, all closed — spec §5).** Several designs: unchanged. Not done, owner's word needed: writing the design into the Transactions row for a single-design bundle (§3). **Owner: open a bundle sale of 77019 to ABBA → Step 2 shows `1,450/yd — last paid by ABBA` as the first chip; a design she never bought shows the `No earlier sale` line.** | **Owner** | `telegram-ops-bot/specs/RATE-1_LAST_PAID_CHIP.md` |
| **0 — OWNER LIVE CHECK** | SFS-1 SHIPPED 22-Sep-2026 (owner: "I want to see all the sales which have taken place from selected warehouses/stores/office" … "just the tappable chips for existing warehouses or the stores on screen 1 … No more functionalities, no more unnecessary items"). 📊 Reporting → 🏬 **Store Sales**, admin-only, three read-only screens: one plain chip per place that has sold anything (alphabetical) → the 📒 Customer Supplies day-chip grammar with the place in the title (`🏬 Sales — IDUMOTA` · `Total: 1B + 5t · 224 yds` · `19 Aug 2026 — 5t (164 yds)`, 8 per page) → the day's card, customer → design → `Shade X ×N (bale numbers)`, quantities only, ⬅ Dates / ❌ Close. Read-time grouping of sold Inventory rows (every sold than keeps the warehouse it was sold from) — nothing written, no column, no knob. A blank-warehouse sold row forms no chip (ruling owed: an `Unassigned` chip?); an oversize day ends `…and N more lines`; a returned than drops off its day (as Customer Supplies). **Hardened the same day after an adversarial review (14 findings closed, spec §7):** one bale prints once even when its thans spell the shade two ways, rejected date cells keep their text, names bolded without backslashes, ❌ Close edits the card in place, screen 1's 🏠 Back to menu (`sfs:menu`) ends the session BEFORE the message becomes the menu (the janitor would otherwise delete the menu — a house-wide `act:__back__` gap, ask-first), a tile tapped over a live flow hands it off. **Owner: open 🏬 Store Sales → IDUMOTA → one day; confirm the header total agrees with 📒 Customer Supplies for the same days and that nothing but the listed buttons appears.** | **Owner** | `telegram-ops-bot/specs/SFS-1_STORE_SALES.md`; `src/flows/storeSalesFlow.js` |
| **0 — OWNER LIVE CHECK** | SSA-1 SHIPPED 24-Sep-2026 (owner: "grant access to see the sales from different warehouses to different employees … through checkboxes"; rulings: immediate · tell the employee · scope 🏬 Store Sales + 📒 Customer Supplies together, 📦 Supply Details left for later · nothing until ticked). 👥 Human Resources → 🔐 **Sales Access** (admin-only): tap a person (chips show what they hold) → tick places (every warehouse/store the bot knows) → ✅ Save writes **Users column L `store_sales_places`** (one trailing header cell, bootstrap-added; separate from column I `warehouses`, the supply-door scope), logs `sales_access_updated`, DMs the person when they GAIN access (a removal is silent — owner, same day). The grant decides the MENU (a grant adds both tiles, none removes both — CSV or not) and the DATA (Store Sales: one place opens straight on its day tiles; Customer Supplies: customers, days, cards and the sale-doc chip filtered to rows sold FROM the ticked places). A failed Users read scopes to nothing. Locked as BUSINESS_RULES §18. **Owner: 🔐 Sales Access → Abdul → tick Kano office → Save; on Abdul's phone check 📊 Reporting shows both tiles, Store Sales opens on `🏬 Sales — Kano office` with no Change place, Customer Supplies lists only Kano buyers; untick → Save → both tiles gone, he gets NO message; the Users sheet gained exactly one trailing header `L = store_sales_places`.** Owed: your word on scoping 📦 Supply Details and the stock reports the same way. | **Owner** | `telegram-ops-bot/specs/SSA-1_SALES_ACCESS.md`; `src/services/salesAccessService.js`; `src/flows/salesAccessFlow.js` |
...
| **0 — OWNER STEPS** | TRF-20 one transfer — **steps 1–6 SHIPPED 22-Sep-2026** (your go; D4 = `18Sep·02 · 🔴 LAG▸KAN · 10B`): load identity + ghost census/decline scripts; idempotent reference + the Send guard (Open / Back / admin Send anyway ⧉); one row on the 🛂 inbox, 📋 list and My Tasks; hourly holder nag + `TRANSFER_STALE_DAYS`; legacy tap doors deleted; Postgres `transfers` shadow (migration 003) + parity script. **Owner, in order: (1) `node scripts/list-ghost-transfers.js`; (2) `node scripts/decline-ghost-transfers.js --as <your id> --commit` and tell the requesters it names; (3) confirm the offsite backup job + Railway snapshots; (4) after a few days `node scripts/transfers-parity.js` → PARITY OK → say the word for the read flip (step 7) and the sheet mirror (step 8); (5) rule on D8 — do disjoint pinned bale numbers make two orders different loads?** | **Owner** | `telegram-ops-bot/specs/TRF-20_ONE_TRANSFER.md` §7 |
...
| **0 — OWNER LIVE CHECK** | SEN-2 SHIPPED 24-Sep-2026 (owner: "only send it when something is wrong … keep the other items with a tappable option"): the 20:00 🩺 Data Health DM was already silent when all eleven checks pass; it now arrives COLLAPSED — `🩺 Data Health — 2 issue(s) found`, the failing checks with their lines, then `✅ 9 other checks clean` — with `🔽 Show all 11` (redraws the same night's result in place, session-free; after a restart it re-runs the checks once) / `🔼 Show issues only` and `🩺 Open Data Health`. The tile itself is unchanged. `SENTINEL_ENABLED = 0` in Settings still switches the nightly run off entirely; `SENTINEL_HOUR` moves it. Tonight's card will keep arriving until C5 ("Madam Motunrayo" on 30 sold rows matches no customer — 🔀 Merge Customers or ➕ Add Contact) and C10 (one bale_uid on rows 2839–2840 — ✏️ Edit Bale) are fixed. **Owner: tonight, tap 🔽 Show all on the card and 🔼 back.** | **Owner** | `src/services/consistencySentinel.js` (`buildReport`, `reportKeyboard`), `src/flows/dataHealthFlow.js` (`snt:dm:*`) |
...
| **0 — OWNER LIVE CHECK** | SELL-T3c SHIPPED 25-Sep-2026 (owner: "it is asking me to select the thans available in that bale manually tapping … I want this to be auto selected with option to make changes as existing"): in a typed sell line that mixes bare bales and `<bale>/<than>` tokens (`Sell 771,779,775,773,772,6189/5,6210/5,6199/5 to AbdulGayinu`), each bare bale now loads EVERY available than of that bale into the cart (as `📦 Take whole bale` does), the review card names the bales loaded whole, and their `🔎 Open` chips open the than chips TICKED so one can be dropped; `771 x3` still opens unticked (§2 — three of six is his choice); a bare-only line stays the SELL-T1 door. Rule §2 carries the clarification. Enhancement list kept aside in the spec (typed customer as a suggested chip at approval · `x3` straight to the bale card · `(1 dropped)` on the header count · `· whole bale` short lines · bare-only lines through the local parser · Lagos confirmation). **Owner: send your line as typed; confirm the card shows the five bales with all their thans and one 🔎 Open chip each, open one, drop a than, Back, then ✅ Continue.** | **Owner** | `telegram-ops-bot/specs/SELL-T3_TYPED_THAN_LIST.md` (SELL-T3c); `src/flows/bundleSaleFlow.js` (`startWithThans`) |
...
- Flow modules: `gr:` `br:` `addstock:` `pr:` `wh:` `wai:` `edb:` (EDB-1 Edit Bale) `bs:` `udf:` `sbl:` (📒 Customer Supplies — since SSA-1 scoped to the person's ticked places) `ssa:` (SSA-1 🔐 Sales Access: `u:<i>` person · `p:<i>` toggle · `save` · `back` · `close` · `menu`) `lcost:` `bops:` `ofex:` `usr:` `umg:` `rol:` `atd:` `atd_rpt:` `atd_adm:` `tsk:` `nf:` `swv:` `pp:` `pay:` (PAY-1 payments; `pay:done|dec` are
...
| `src/services/salesAccessService.js` | SSA-1 — which places' sales a person may see (admin: all; else Users column L; none = nothing): `scopeFor`, `filterRows`, `adjustMenu` (menu injection), `grant` |
...
Users column L = `store_sales_places` (SSA-1, 24-Sep-2026) — the one trailing
column after `notification_prefs`: CSV of the places whose SALES this person may
see in 🏬 Store Sales / 📒 Customer Supplies, written by 🔐 Sales Access (admin,
immediate). Separate from column I `warehouses` (the supply-door scope). Admins are
never scoped; an empty cell = the tiles are hidden (BUSINESS_RULES §18).
`schemaMapper` adds the header cell to a live 11-column sheet and touches no data row.
