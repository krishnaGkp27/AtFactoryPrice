...
| **0 — OWNER RULING** | ISC-1 Inventory sheet cleanup — ANALYSIS + PLAN only (08-Sep-2026) from the 04-Sep PDF: 105 sold rows carry a text SoldDate (`cashmere …`) the bot cannot date; 2,853 legacy rows have no `bale_uid` so **sorting the sheet re-keys them — do not sort until the backfill runs**; 3 customer spelling clusters (Awunawu ×3, Oshodi ×4, Ketu ×2); 374 SOLD rows at ₦0; 64 oversize thans for EDB-1; one duplicate uid on 77014/864. Owner: run `scripts/format-date-columns.js --commit` (still pending since 14-Aug), answer R1–R11 in the spec, then the guarded one-offs ship in order. | **Owner** | `telegram-ops-bot/specs/ISC-1_INVENTORY_SHEET_CLEANUP.md` §4–§5 |
| **0 — OWNER LIVE CHECK** | CUR-2 invoice rate multiplier — **release A SHIPPED 09-Sep-2026** on the Settings-fulfilled path: Settings `INVOICE_RATE_MULTIPLIER` (env of the same name = boot default; blank / 0 / 1 = none), new trailing `Invoices.rate_multiplier` column W frozen at issue, the document (PDF + `/i/<token>`) rendered from stored BASE lines × stored factor (rate 2 dp, integer amounts, payment row and DEBIT BALANCE at the same factor, status from the base figures, no symbol or unit), staff caption = base figures only — the factor is INTERNAL (owner 09-Sep: shown only while entering it; never on the document, caption, replies or cards); the sheet row never holds a multiplied figure (owner's 09-Sep integrity ruling, §17). **Owner: set the cell to `1250` → approve one small sale → open the invoice link and the PDF → compare with the base figures on the approval reply and the statement → blank the cell and confirm the issued invoice does not change** (full steps: CUR-1 spec §11 / TESTING.md §3.2a). **Release B (pending your go after that check):** the wizard's Step 5 in `approvalEvents.js` + `INVOICE_MULTIPLIER_ASK`, and the §17 closing sentence you approve verbatim (recorded there as PENDING). | **Owner, then agent** | `telegram-ops-bot/specs/CUR-2_INVOICE_MULTIPLIER.md` status block, §7, §9 |
| **0 — OWNER LIVE CHECK** | CUR-1 currency display — rulings R1–R18 / D1–D6 closed "as recommended" 08-Sep; **release A SHIPPED 09-Sep-2026** (build steps 1–5): `src/utils/money.js` (`expense` → `₦12,345` · `sale` → `12,345` · `saleRate` → `1,450/yd` · `code()`), `format.js` shims (deprecated, behaviour-identical), side A pinned (task incentives store the literal `NGN`, R11), the invoice bare in both states, side-B flows/services swept (sales browser, sold bales, catalogue, rate suggestions, warehouse audit, landed cost, return credit, the Record Payment / return / Owes cards in `approvalCards`, `/balance` `/ledger`, reports engine), S-CUR smoke lint in warn mode. **Owner: live check = the CUR-2 row above plus side A (one expense, the 20:00 report, one PAY-1 request, one incentive — every figure still `₦`) and side B (Stock Value, Check Stock `Selling: 1,500/yd`, catalogue `· 1,500/yd`, Return goods card, `/balance` — all bare).** **Release B (pending your go):** the `approvalEvents.js` chip / ack / Outstanding-line sweep (step 6 — its `approvalCards.js` half shipped in A), the `telegramController.js` sweep (step 7), deleting the `format.js` shims + S-CUR in fail mode (step 9). Until then the "📒 Outstanding" line under an approved sale still says `NGN …` and buyer chips still show `₦…/yd` — by design, not a defect. | **Owner, then agent** | `telegram-ops-bot/specs/CUR-1_CURRENCY_DISPLAY.md` §11, §8 |
