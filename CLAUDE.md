...
| **0 — OWNER RULING** | PAY-3 the finance phone — PROPOSAL only (14-Sep-2026). Owner sent the payment approval card as it looks on the office phone, then: **"I don't want to make this approval too long."** So the fix ADDS the date and the signer while REMOVING lines — every item is a cut, not an addition. **C1** payee+account merge to one line; **C2** the footer `Requested by Abdul · 1d ago · R-4F72` → `Abdul · 13 Sep 11:39 · 1d · R-4F72` (2 chars shorter, gains the DATE the card never had); **C3** the 74-char sentence `1 of 2 approvals already given — a different admin must give the second` (wraps to three lines on a phone) → `⚠️ 1 of 2 · signed by Musa` (one line) — the dropped sentence is redundant, since `1 of 2` says a second is needed and the reader IS the different admin; when the existing signature is YOUR OWN the ✅ chip is simply not offered (the guard refuses that tap anyway — the missing button is the message, no rule changes). **His card: ~10 rendered lines today → 6.** The DM copy is fatter still (12 → 7): it prints the raw UUID as `Ref:` plus two content-free lines (`Sent for approval`, `Use buttons below to approve or reject`). **C2 + C3 are shared by EVERY inbox category** — one line of code each — so every approval card gets the date and gets shorter in one vocabulary; say the word if you want payments only. **Menu answer (no change needed): approve in 🛂 Approvals → 💳 Payments, pay in 💰 Finance → 💳 Payments → 💳 Waiting for me to pay — never both at once.** The two `approvalEvents` items (DM card + alert) are ask-first. **R1 owed: may the admin who gives the second signature also be the one who pays?** Today yes — `canExecute` checks only finance-seat membership. (a) leave it *(recommended at four people)*, (b) the finance seat may never sign, (c) it may give the first signature but never the second. | **Owner** | `telegram-ops-bot/specs/PAY-3_FINANCE_PHONE.md` §2, §4 |
