...
| **0 — OWNER GO** | TRF-20 one transfer — SPEC 22-Sep-2026 (owner: duplicate transfer chips, "fix the structure … migrate onto PostgreSQL"). Data: 32 of 67 transfer rows are identical twins, 8 of 16 open rows are ghosts, stuck transfers never nag. Design: one door (legacy `tp*`/`tt*` doors deleted), idempotent identity, duplicate guard at Send, one row grammar on the list / inbox / My Tasks / DM, hourly nag + `TRANSFER_STALE_DAYS`, ghosts declined by script, then Postgres `transfers` table as the pilot (shadow → parity → read flip → mirror). **Owner: confirm D1–D7 in the spec §2, decline the 21-Sep test transfer TR-20260921-001, and confirm the backup job ships first.** | **Owner** | `telegram-ops-bot/specs/TRF-20_ONE_TRANSFER.md` |
