...
| 16-Sep-2026 | Audit checklist: a reconciled (green) chip shows the figure the auditor entered that day, ADMINS ONLY — `✅ 9006 · 12B + 3t · 20-Aug-26` (rule-6c grammar). An auditor's chips stay blind (WAU-3): a green figure IS today's book figure and the full re-audit sheet re-counts green designs. Owner: "Go ahead with admins only." | AUD-C1 (`src/flows/warehouseAuditFlow.js` `doneChipLabel`) |
| 22-Sep-2026 | Transfers (TRF-20, owner go): ONE load = route + multiset of (design, shade, qty), whoever raised it; an identical OPEN transfer refuses a second Send (admins may "Send anyway", stamped ⧉ duplicateOf); the reference is minted when the confirm card is drawn and a retry can never write a second row; every transfer row reads `18Sep·02 · 🔴 LAG▸KAN · 10B` (date · dot · route · quantity — no design, "the row will get crowded"); a stuck transfer nags its holder hourly and every admin after `TRANSFER_STALE_DAYS`; the legacy single-bale/than tap doors are deleted; ghosts (open pre-dispatch twins of a transfer that went further) are declined by script, never by hand-editing the sheet; a declined transfer names the decliner in Approver column H. PENDING D8: whether disjoint pinned bale numbers make two orders different loads. | TRF-20 (`specs/TRF-20_ONE_TRANSFER.md`) |
| 23-Sep-2026 | A customer is removed INSIDE the bot, behind two admins, never by editing the sheet: ➖ Remove Customer (CRM hub) → reason → card disclosing outstanding balance (never a gate), supplies on record and network children → second admin approves → status `inactive` + dated note on the Customers row and its Contacts node; no row is ever deleted; ↩️ Restore is the same door. Owner: "make a provision inside the bot to deactivate the customer with two admin approvals." | RMV-1 Phase B (`src/flows/customerRemoveFlow.js`) |
