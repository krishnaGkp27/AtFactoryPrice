...
| 12 | **APX-5 sweep — DONE 31-Jul for the convertible set.** addStockFlow (12 outcomes now edit one status card), photoReceiveFlow (OCR progress rides the anchor), bulkReceiveFlow (error + expiry take over the tapped card). Triaged as LEGITIMATE sends, left alone: warehouseAuditFlow (offline count sheet must be a copyable standalone message; batch results reply to typed submissions), attendance flows (reply-keyboard GPS mechanics require fresh messages), goodsReceipt/userAdd typed-validation warnings (low-sensitivity typed replies). | done / exempt |
| 12h | **Inventory sheet may carry two orphan headers — X1 `prev_state`, Y1 `state_since`.** Commit `480d46e` (03-Aug) briefly self-created them at boot before the owner ruled "don't add any unnecessary columns in inventory sheet"; `cffa58a` reverted the code, but a deploy that ran in between could have written the two header cells. Nothing writes to them now and no reader looks at them, so they are cosmetic — but they should go. Either the owner clears X1/Y1 by hand, or an agent adds a guarded one-off that deletes the two header cells ONLY when both columns are empty below row 1. Owner asked to be reminded. | owner / agent — owner asked for a reminder |
| 12g | **Hide ₦ outside Finance (owner, 02-Aug — "keep it in mind, don't implement for now").** Sale value/rate becomes a Finance-department visibility, not shown on the supply/report cards. Today the gate is `pricingService.canSeeSalePrice` / env admins. Waiting on the owner's go. | agent, when owner calls it |
| 12f | **TV-8 sweep — the ~25 surfaces still printing a hardcoded unit.** TV-8 (02-Aug) fixed the three supply/sales-history drills; Sales Report, Check Stock, List Bales, Inventory Details, Stock Value, Sales Browser, the supply-statement PDF and the customer reports still print `N Bales · M thans` (both units, banned by BUSINESS_RULES §6c) or bales-only regardless of store. Several drop the warehouse at aggregation time (`buildSalesDesignReport`, `buildCustomerPattern`, `stockValueReport`), so their grouping key must be extended before a warehouse-aware unit is even expressible. Also `queryEngine.freeFormQuery`'s system prompt orders the model to "always include package counts, than counts, and yards" — that prompt must change too, or the AI analyst keeps violating the rule. | agent, next reporting pass |
| 12e | **DONE 02-Aug (TRF-INT4).** Sale/return executors are warehouse-scoped: every sale flow stamps the selling warehouse onto its queue items, executors + price stamps act only in that warehouse, typed returns get a warehouse chip when the number was sold in two places, and the Return-Than picker is one-chip-per-physical-bale. Pre-change pending approvals execute the legacy status-guarded way. Spec: specs/TRF-INT_BALE_INTEGRITY.md. | shipped |
| 12d | **TRF-13 (owner, 01-Aug: "will be doing it later") — reconcile the dispatch PDF/photo against the logged bales.** Today the doc is archived + viewable but never machine-read. Prior art ready: snapSale OCR reads bale labels from photos; the transfer row carries both the doc fileId and the per-line bale numbers (TRF-12), so an OCR pass can diff "doc says vs sheet says" and flag mismatches on the receiver card before ✅ Received. | agent, when owner calls it |
| 12c | **SJ-4 follow-up — flows the aux-disposal upgrade skipped.** SJ-4 (31-Jul) tracks/deletes interim prompts + catalogue photo cards at completion/cancel for sellBale→sale pipeline, supply request, photoReceive edit trail, goodsReceipt warnings, transfer doc gate; seals sale/srf/supplyStatement terminals in place. Still open: **receipt_flow has no anchor at all** (startReceiptFlow drops its messageId — whole prompt trail unsealed; needs the flowKit.makeRenderer conversion), and the legacy srf cart/step cards themselves are unanchored (each step still sends a fresh card; only the one-off prompts/nags are swept). Exempt by design: approval cards + attachments, DM broadcasts, users' own uploads. | agent, next polish pass |
