...
| 12e | **Sale executors: warehouse scoping (TRF-INT follow-up).** `sell_package` still keys by printed number + status only — a cross-warehouse duplicate sold by typed sale can flip both warehouses' bales. TRF-INT1 fixed transfers/returns; sales need the selling warehouse to ride the sale items from the flow session into the executor (sellBale knows session.warehouse; snap aj already carries it). Small, separate task per specs/TRF-INT_BALE_INTEGRITY.md. | agent, next integrity pass |
